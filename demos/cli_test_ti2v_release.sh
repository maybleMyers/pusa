...
# for CHECKPOINT in "${CHECKPOINTS[@]}"; do
#     CUDA_VISIBLE_DEVICES=0,1,2,3 python3 ./demos/cli_test_ti2v_release.py --model_dir "$MODEL_DIR" --dit_path "$CHECKPOINT" --image_dir "$image_dir" --prompt "$prompt" --prompt_dir "$prompt_dir" --cond_position "$cond_position" --num_steps "$num_steps" --noise_multiplier "$noise_multiplier"
# done
