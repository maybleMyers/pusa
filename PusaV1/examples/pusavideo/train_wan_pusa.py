...
class LightningModelForTrain(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        timestep_id = torch.randint(0, self.pipe.scheduler.num_train_timesteps, (latents.shape[0], latents.shape[2])) #TODO
        ...
        if self.use_wandb:
            self.step_count += 1
            if self.step_count % 10 == 0:  # Log every 10 steps to avoid too many logs
                wandb.log({
...
def train(args):
    ...
    if args.use_wandb:
        try:
            ...
            wandb.init(
                project="wan-video",
                name=f"wan-training-{os.path.basename(args.output_path)}-{timestamp}",
                config=vars(args),
            )
            if args.verbose:
